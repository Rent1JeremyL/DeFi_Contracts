...
import "./NarwhalToken.sol";

// MasterChef is the master of the Governance Token. He can make Governance Token and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Governance Token is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        require(_depositFeeBP <= 500, "add: invalid deposit fee basis points"); // Max 5%
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingGovToken(uint256 _pid, address _user) external view returns (uint256) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 tokenReward = multiplier.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        govToken.mint(devaddr, tokenReward.div(10));
        govToken.mint(address(this), tokenReward);
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = govToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBal) {
            transferSuccess = govToken.transfer(_to, tokenBal);
        } else {
    ...
    // update start block, how many blocks
